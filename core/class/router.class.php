...
class Router {
	...
	public function getStatus() {
		$state = $this->getState();
		//log::add('huawei_dongle', 'debug', 'State: '.$state['State']);

		if(empty($state['State'])) {
			if(intval($state['State']) == self::LOGGED_IN) {
				$this->statut = "Up";
				//log::add('huawei_dongle', 'debug', 'Up');
			} else {
				$this->statut = "Down";
	...
	*/
	...
	private function setSMSPython($tel, $msg) {
		...
		$command = dirname(__FILE__) . '/../../resources/scripts/sender.py '.$this->getIP().' '.$this->getLogin().' '.$this->getPassword().' '.$tel.' '.mb_convert_encoding($string, "UTF-8", mb_detect_encoding($escapedArg, "UTF-8, ISO-8859-1, ISO-8859-15", true));
		try{
			$json = shell_exec('python3 '.$command);
		} catch (Exception $e){
			log::add('huawei_dongle', 'debug', $e);
